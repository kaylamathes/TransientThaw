...
predicted_thaw_rate_difference_upland2 <- read.csv("Output/UpdateWeibullPercentChange_Range/WeibullModelOutput_upland_percentChange_Range.csv")
...
options(scipen = 999)
predicted_ALT_mean <- predicted_thaw_rate_difference_upland2%>%
  dplyr::select(tsf, predicted_mean_percentchange_ALD)%>%
...
predicted_ALT_mean <- predicted_ALT_mean%>%
...
predicted_ALD_allDOY_mean <- predicted_ALT_mean%>%
  expand(tsf, full_seq(Predicted_DOY_list,1))


##Join that list of DOY across all tsf 
predicted_ALD_allDOY_mean <- left_join(predicted_ALD_allDOY_mean, predicted_ALT_mean, by = c("tsf"))%>%
...
write.csv(predicted_ALD_allDOY_2_mean, "Output/UpdateWeibullPercentChange_Range/Predicted_ALD_change_upland_percentchange_mean.csv")
...
ggsave("Output/UpdateWeibullPercentChange_Range/Predicted_ALD_change_upland_percentchange_mean.png", height = 10, width= 10)


### Max - High Scenario 
##Now take the predicted end of season ALT depths from the modelled data and add the average end of year DOY 
options(scipen = 999)
predicted_ALT_max <- predicted_thaw_rate_difference_upland2%>%
  dplyr::select(tsf, predicted_max_percentchange_ALD)%>%
...
##Find a parameter (the quadratic parameter: use the average across the tsf that we do have: cannot predict this shape of the parabula for each other year without more Doy data)
predicted_ALT_max <- predicted_ALT_max%>%
  mutate(a = case_when(tsf >= 0 & tsf <= 4 ~ a1, 
                       tsf > 4 & tsf <= 49 ~ a_0_4_mean,
                       tsf > 49 ~ 0))%>%
  group_by(tsf)%>%
...
predicted_ALD_allDOY_max <- predicted_ALT_max%>%
  expand(tsf, full_seq(Predicted_DOY_list,1))


##Join that list of DOY across all tsf 
predicted_ALD_allDOY_max <- left_join(predicted_ALD_allDOY_max, predicted_ALT_max, by = c("tsf"))%>%
  rename(Predicted_Doy = "full_seq(Predicted_DOY_list, 1)")
...
write.csv(predicted_ALD_allDOY_2_max, "Output/UpdateWeibullPercentChange_Range/Predicted_ALD_change_upland_percentchange_max.csv")
...
ggsave("Output/UpdateWeibullPercentChange_Range/Predicted_ALD_change_upland_percentchange_max.png", height = 10, width= 10)
...
##Now take the predicted end of season ALT depths from the modelled data and add the average end of year DOY 
options(scipen = 999)
predicted_ALT_min <- predicted_thaw_rate_difference_upland2%>%
  dplyr::select(tsf, predicted_min_percentchange_ALD)%>%
...
##Find a parameter (the quadratic parameter: use the average across the tsf that we do have: cannot predict this shape of the parabula for each other year without more Doy data)
predicted_ALT_min <- predicted_ALT_min%>%
  mutate(a = case_when(tsf >= 0 & tsf <= 4 ~ a_13_49_mean, 
                       tsf > 4 & tsf <= 49 ~ a17,
                       tsf > 49 ~ 0))%>%
  group_by(tsf)%>%
...
predicted_ALD_allDOY_min <- predicted_ALT_min%>%
  expand(tsf, full_seq(Predicted_DOY_list,1))


##Join that list of DOY across all tsf 
predicted_ALD_allDOY_min <- left_join(predicted_ALD_allDOY_min, predicted_ALT_min, by = c("tsf"))%>%
  rename(Predicted_Doy = "full_seq(Predicted_DOY_list, 1)")
...
write.csv(predicted_ALD_allDOY_2_min, "Output/UpdateWeibullPercentChange_Range/Predicted_ALD_change_upland_percentchange_min.csv")
...
ggsave("Output/UpdateWeibullPercentChange_Range/Predicted_ALD_change_upland_percentchange_min.png", height = 10, width= 10)
